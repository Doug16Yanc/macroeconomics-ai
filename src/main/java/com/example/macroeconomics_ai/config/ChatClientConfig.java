...
import com.example.macroeconomics_ai.tool.MacroeconomicsTools;
    ...
    public ChatClient chatClient(ChatModel chatModel, MacroeconomicsTools macroeconomicsTools) {
        return ChatClient.builder(chatModel)
                .defaultSystem("""
                    ...
                    8. LABOR MARKET / TECH SECTOR CHAINING RULE: whenever a question mentions
                       the "job market," "tech/IT hiring," or "labor market," you MUST call
                       getLaborMarketSeries for EACH of the following series, in this order,
                       before answering:
                       - ICSA (weekly jobless claims) — short-term signal
                       - JTSJOL (job openings, JOLTS) — monthly hiring trend
                       - PAYEMS (nonfarm payrolls) — broad employment context
                       Then also call fedInterestRate, since sustained high rates reduce
                       venture-funded hiring appetite in tech more than in the broader economy.
                       Example (follow this pattern exactly):
                       Question: "Is the slowdown in tech hiring a broad economic phenomenon
                       or something isolated to the sector?"
                       Expected action: call all four tools above, then reason as follows —
                       if PAYEMS keeps growing but JTSJOL falls specifically in a way that
                       tracks tech-sector reporting, and ICSA does not spike broadly, this
                       points to "fewer hires, not more layoffs" — a sector-specific slowdown
                       rather than a broad-based one. If ICSA rises broadly and JTSJOL falls
                       across sectors, this points to a macro phenomenon instead. Present this
                       reasoning explicitly in a "Sector-Specific vs. Broad Phenomenon" section,
                       grounded only in the values returned by the tools.
                    9. Always respond in English, regardless of the language the question was
                       asked in. If the user writes in Portuguese, Spanish, or any other
                       language, still produce the entire report — headline, tables, and
                       analysis — in English.
                """)
